...
import (
	...
	sc "github.com/argotnaut/vanitea/stackcontainer"
...
func main() {
	...
	// 	},
	// )
	stackContainer := sc.NewStackContainerFromComponents(
		[]*con.Component{
			con.ComponentFromModel(
				lc.NewLinearContainerFromComponents(
					[]*con.Component{
						...
						con.ComponentFromModel(
							placeholder.GetPlaceholder(&styleYellow, nil, nil, nil),
						).SetTitle("yellow").SetShowTitle(true),
						con.ComponentFromModel(
							placeholder.GetPlaceholder(&styleLavender, nil, nil, nil),
						).SetTitle("lavender").SetShowTitle(true),
					},
				).SetDirection(lc.HORIZONTAL), // re-specifying the default orientation here for clarity
			).SetFocusable(false).SetBorderStyle(con.NO_BORDER_STYLE).SetTitle("none").SetShowTitle(false),
			con.ComponentFromModel(
				placeholder.GetPlaceholder(&styleMagenta, nil, nil, nil),
			).SetTitle("magenta").SetShowTitle(true).SetMaximumHeight(10).SetMaximumWidth(40),
		},
	)

	_, err := tea.NewProgram(stackContainer, tea.WithAltScreen()).Run()
	if err != nil {
		panic(err)
	}
}
